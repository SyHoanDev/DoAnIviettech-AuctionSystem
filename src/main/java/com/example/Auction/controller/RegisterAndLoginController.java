...
public class RegisterAndLoginController {
    ...
    public String registerAcc(AccountEntity account, @RequestParam(value = "passwordConfirm")String passConfirm,
                              BindingResult bindingResult, Model model) throws Exception {
        if (account.getPassWord().length() < 8) {
            model.addAttribute("successMessage","Vui lòng sửa lại biểu mẫu!");
            model.addAttribute("error","Mật khẩu có ít nhất 8 ký tự");
        }else if (account.getPassWord().equals(passConfirm)) {
